lock_media = {}-- An empty table for solving multiple kicking problem
...
local function run(msg, matches)
  ...
  if data[tostring(msg.to.id)]['settings']['lock_media'] then
    if data[tostring(msg.to.id)]['settings']['lock_media'] == 'yes' then
      ...
      if lock_media[msg.from.id] == true then 
        return
      end
	  if msg.to.type == 'chat' then
		...
		if username and is_super_group(msg) then
			send_large_msg(receiver , "Media is not allowed here\n@"..username.." ["..msg.from.id.."]\nStatus: User kicked/msg deleted")
		else
...
local function cron()
  lock_media = {} -- Clear lock_media table 
end
